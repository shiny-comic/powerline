...
def compile_client():
    ...
    if hasattr(sys, "getwindowsversion"):
        ...
        subprocess.check_call(
            compiler
            + shlex.split(cflags)
            + ["-std=c11", "client/powerline.c", "-o", "scripts/powerline"]
        )
...
setup(
    ...
    extras_require={
        ...
        "volume segment": ["pyalsaaudio"],
        "docs": [
            "Sphinx",
            "sphinx_rtd_theme",
        ],
        "appoints segment support, Google Calendar": ["google-api-python-client"],
        "cpu load segment support": ["psutil"],
        "wifi segment support": ["iwlib"],
        "better git support in vcs segment": ["pygit2"],
    },
)
